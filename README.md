...
- 프로젝트를 진행하면서 느낀 점
  - 어려웠던 점
    ...
    **`1. 포톤을 이용한 멀티플레이 개발`** ⬅️
      게임에 멀티플레이 기능을 넣고자 한 프로젝트는 이번이 처음이었습니다. 자료조사를 했을 때, 유니티 구버전을 기준으로 한 것들도 많아 헷갈렸지만, 그래도 문제없이 해낼 수 있었습니다.
      
    **`2. 유니티 머신러닝 에이전트를 이용한 인공지능`** ⬅️
      유니티 머신러닝 에이전트를 통하여 크립 몬스터를 기획 및 개발하였습니다. 시연동영상에서 보여지는 골렘의 경우 코딩으로 행동패턴을 정의한 것이 아닌, 기계학습을 통하여 학습시킨 인공지능입니다. 관련된 학습자료도 적고, UMA라이브러리 자체도 프로젝트 초기이기 때문에, 많은 어려움이 있었습니다. 개발 단계에서도 많이 학습시키는데 많은 시간과 데이터가 들어가 프로젝트에 결합시킬 때도 힘들었습니다.
